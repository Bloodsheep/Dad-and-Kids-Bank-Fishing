...
const spots = [
  {
    ...
    source:"https://tpwd.texas.gov/fishboat/fish/recreational/lakes/houston/",
  ...
  },
  {
    ...
    source:"https://tpwd.texas.gov/fishboat/fish/recreational/lakes/houston/",
    summary:"Very family-friendly public lake access with fishing piers/shoreline, parking, picnic space and room for the kids. The big tradeoff: park hours clip both your earliest morning and latest evening windows.",
    ...
    hours:"Typically 6 AM–7 PM",
    openStart:6, openEnd:19,
    access:[
      "Public park with fishing pier/shoreline access.",
      "Practical morning window: about 6–10 AM. Practical evening window: about 5–7 PM.",
...
function calcAccessWindow(s){
  const desired=period==="morning"?[5,10]:[17,22];
  const start=Math.max(desired[0],s.openStart);
  const end=Math.min(desired[1],s.openEnd);
...
function scoreConditions(s, datum, dateObj){
  ...
  const window=calcAccessWindow(s);
  if(!window) score-=35;
  else {
...
async function fetchWeather(s,dateObj){
  ...
  const url=`https://api.open-meteo.com/v1/forecast?latitude=${s.lat}&longitude=${s.lon}&hourly=temperature_2m,precipitation_probability,wind_speed_10m&temperature_unit=fahrenheit&wind_speed_unit=mph&timezone=America%2FChicago&start_date=${date}&end_date=${date}`;
  try{
    ...
    const d={temp:avg("temp"),rain:Math.max(...vals.map(v=>v.rain??0)),wind:avg("wind")};
...
function renderSpot(s,dateObj,weather){
  ...
  $("#accessList").innerHTML=[`<strong>Posted/typical access:</strong> ${s.hours}`,...s.access].map(x=>`<li>${x}</li>`).join("");
